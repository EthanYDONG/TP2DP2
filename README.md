...
- **RMTPP_dpp_main.py**: The main script for running experiments with Bayesian mixture of point process with DPP prior. This script initializes the model, loads the data, and evaluates the performance.
- **RMTPP_dpp_train.py**: This script handles the training of the Bayesian mixture of point process model with DPP.

### Models

- **RMTPP.py**: Implementation of the neural network-based point process model.
- **RMTPP_dpp.py**: Extension of the neural network-based point process model with a Determinantal Point Process (DPP) prior.
- **HawkesModel.py**: Implementation of the parametric Hawkes process model.
...
- **rmtpp_embed.py**: Provides embedding utilities for the Bayesian mixture of point process model.
